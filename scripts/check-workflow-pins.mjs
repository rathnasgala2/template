...
async function main() {
  ...
  try {
    ...
    // This repository has no `.github/workflows` directory yet. That is not
    // a violation of the pin policy (there is nothing to pin), so the check
    // passes trivially rather than throwing ENOENT. Any other error still
    // propagates.
    if (error?.code === 'ENOENT') {
      process.stdout.write(
        'Verified 0 workflow file(s): .github/workflows is absent.\n',
      );
      return;
    }
    throw error;
