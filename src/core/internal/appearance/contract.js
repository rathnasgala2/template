/**
 * The single module `contracts/theme-styling-contract.jcs` targets for
 * every Light/Dark/System appearance name this renderer owns: the
 * `data-` attribute/class names the controller sets must be fixed
 * constants exported from one module. The published
 * `templateStylingContract` catalog's closed theme-package and stylesheet
 * admission requires the root attribute row
 * `{name:data-gala-publication-root,match:presence,values:[],role:root}` and
 * the palette attribute row
 ...
 *
 * Two attributes are exposed, deliberately kept separate: selection and
 * resolved state are exposed separately through template-owned root
 * attributes; theme CSS consumes only resolved state.
 *
 * - {@link APPEARANCE_ROOT_ATTRIBUTE} and {@link APPEARANCE_RESOLVED_MODE_ATTRIBUTE}
 *   are the two fixed theme-facing attributes above — part of the published
 *   styling contract, never renamed here independent of that contract.
 ...
 *   is not a theme-styling-contract hook — the published attribute catalog
 *   does not name it, and no theme selector may target it — but its name is
 *   still fixed here, in this one module, exactly like the two
 *   contract-owned attributes, so no other module ever invents its own
 *   literal for it.
 *
...
/**
 * The `templateStylingContract.publicationRootSelector` attribute:
...
/**
 * The `templateStylingContract.resolvedPaletteSelectors` attribute:
...
/**
 * Every canonical machine-readable selection value: `light`, `dark`,
 * `system`; `lite` is not a value.
 *
 ...
 * `system` is resolved to one of those values rather than becoming a third
 * selector.
 *
...
/**
 * The initial reader default is `system`.
 *
 * @type {'system'}
 */
 ...
 * control's change handler both read/write. Selection is stored under one
 * versioned local key. The trailing `v1` is this key's own version; a
 * future incompatible storage format change mints a new
 ...
 * bootstrap script is written to, entered into the manifest as an ordinary
 * `{path,byteLength,sha256}` asset row. Not content-addressed (unlike the
 * media pipeline's derivative paths): this file's bytes are a pure
 * function of this module's own constants, not
 ...
 * this literal directly (basePath fix: an earlier version of this module
 * used this root-absolute literal unconditionally, which 404s under a
 * non-root `basePath` publication). This constant is
 ...
 * independent of this renderer's own resolved-mode attribute — with
 * JavaScript disabled, CSS media queries preserve `system`.
 *
 * @type {string}
 */
